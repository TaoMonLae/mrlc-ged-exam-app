...
export default function AttemptReview() {
  ...
  return (
    <div className="page">
      <BackBar to={`/teacher/quiz/${data.quiz.id}/submissions`} title="Back to Submissions" />
      <div className="page-header">
        <h2>{data.student.displayName}</h2>
        <div className="small">{data.quiz.title} • Score: {data.attempt.score} / {data.attempt.totalPoints}</div>
      </div>
        ...
        return (
          <div className="card" key={it.questionId} style={{ marginBottom: 12 }}>
            <div className="row" style={{ justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div><strong>Q{idx + 1} ({it.points} pt)</strong> <span className="badge">{it.type}</span></div>
            ...
            <div className="row" style={{ marginTop: 10, gap: 18, flexWrap: "wrap" }}>
              <div style={{ minWidth: 260 }}>
                <div className="small"><strong>Student answer</strong></div>
            ...
            {it.explanation && (
              <div className="small" style={{ marginTop: 10 }}>
                <strong>Teacher explanation:</strong> {it.explanation}
              </div>
            )}
          </div>
        );
