...
router.get("/:id/time", requireAuth, asyncHandler(async (req, res) => {
  if (req.user.type !== "STUDENT") return res.status(403).json({ error: "Students only" });
...
// Autosave
router.patch("/:id/save", requireAuth, asyncHandler(async (req, res) => {
  if (req.user.type !== "STUDENT") return res.status(403).json({ error: "Students only" });
...
router.patch("/:id/override", requireAuth, requireRole(["ADMIN","TEACHER"]), asyncHandler(async (req, res) => {
...
router.get("/results", requireAuth, requireRole(["ADMIN","TEACHER"]), asyncHandler(async (req, res) => {
...
// CSV export
router.get("/results.csv", requireAuth, requireRole(["ADMIN","TEACHER"]), asyncHandler(async (req, res) => {
...
router.get("/:id/review", requireAuth, asyncHandler(async (req, res) => {
  ...
  res.json({
    attempt: {
      ...
      score: finalScore,
      overrideNote: attempt.overrideNote || null,
      totalPoints,
